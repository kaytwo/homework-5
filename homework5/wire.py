...
class CrummyWireProtocol(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data, addr):
        LOGGER.debug(" --> (wire) Received %d bytes from %s", len(data), addr)
        ...
        if len(self._wirebuffer) == self._buffer_size:
            LOGGER.debug(" !-> (wire) Dropping, buffer is full")
            # LOGGER.error("Buffer contents: %s", self._wirebuffer)
            return
        ...
        if self._loss > 0 and random.random() < self._loss:
            LOGGER.debug(" !-> (wire) Dropping to simulate a lossy connection")
            return

        LOGGER.debug(" --> (wire) Added %d bytes to send in %f seconds",
    ...
    def send_to_peer_addrs(self, package):
        ...
        if data not in self._wirebuffer:
            LOGGER.error(" !!! (wire) Was scheduled to send data that is not "
                         "in the write buffer")
            return
        ...
        for a_peer_addr in self._peer_addrs:
            if a_peer_addr == sender_addr:
                continue
            LOGGER.debug(" <-- (wire) Sending %d bytes to %s", len(data),
                         a_peer_addr)
